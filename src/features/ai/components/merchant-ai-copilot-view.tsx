  ...
  return (
    <div className="space-y-6 max-w-6xl mx-auto pb-12">
      ...
      <div className="relative overflow-hidden rounded-2xl border border-line bg-surface p-6 sm:p-8 shadow-md">
        ...
        <div
          aria-hidden="true"
          className="pointer-events-none absolute right-0 top-0 h-full w-full sm:w-2/3 bg-[radial-gradient(circle_at_75%_35%,rgba(196,154,69,0.15),rgba(196,154,69,0.03)_45%,transparent_70%)]"
        />
        ...
        <div className="relative z-10 flex flex-col lg:flex-row lg:items-center justify-between gap-6">
          ...
          <div className="space-y-3 max-w-xl">
            <div className="flex items-center gap-2.5 flex-wrap">
              <span className="inline-flex items-center gap-1.5 px-2.5 py-0.5 rounded-pill bg-surface-sunken border border-primary/40 text-[11px] font-bold text-primary tracking-wider uppercase">
                <Sparkles className="size-3 text-primary" />
                Store Copilot
              </span>
              <span className="text-ink-subtle">·</span>
              <span className="inline-flex items-center gap-1 px-2.5 py-0.5 rounded-pill bg-success-surface text-[11px] font-semibold text-success border border-success/30">
            ...
            <div className="pt-1 flex items-center gap-3">
              ...
              <div className="text-xs text-ink-soft flex items-center gap-1.5">
                ...
                <span>Store telemetry: <strong className="text-ink">{merchantContext.storeName}</strong></span>
              </div>
            </div>
          </div>
          ...
          <div className="shrink-0 flex items-center justify-center lg:justify-end">
            <div className="relative size-44 sm:size-52 rounded-2xl border border-line bg-surface-sunken p-4 shadow-inner flex items-center justify-center overflow-hidden">
              ...
              >
                ...
                <circle
                  ...
                  r="38"
                  fill="none"
                  stroke={isDark ? "#5A4725" : "#D2C2A5"}
                  strokeWidth="1.2"
                />

                {/* Diagonal Connection Vectors */}
                <line x1="45" y1="45" x2="155" y2="155" stroke={isDark ? "#3A3831" : "#E2D7C3"} strokeWidth="0.8" />
              ...
              <div className="absolute bottom-2.5 inset-x-3 text-center">
                <span className="text-[10px] font-mono tracking-widest text-primary uppercase bg-surface px-2 py-0.5 rounded border border-line shadow-xs">
                  AI TELEMETRY • READY
                </span>
              </div>
            </div>
      ...
      {conversation && conversation.messages.length <= 1 && (
        <div className="space-y-3">
          <div className="flex items-center justify-between">
            <h3 className="text-xs font-bold uppercase tracking-wider text-ink-subtle flex items-center gap-2">
              <Lightbulb className="size-3.5 text-primary" />
              Recommended Quick Actions
            </h3>
            <span className="text-[11px] text-ink-soft">Click any card to analyze immediately</span>
          </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3.5">
            {MERCHANT_QUICK_ACTIONS.map((action) => (
              <button
                ...
                disabled={isThinking}
                className="group relative flex flex-col justify-between p-4 rounded-2xl border border-line bg-surface hover:bg-surface-sunken hover:border-primary text-left transition-all duration-200 hover:-translate-y-0.5 shadow-xs disabled:opacity-50 cursor-pointer"
              >
                <div>
                  <div className="flex items-center justify-between mb-3">
                    <span className="text-[10px] font-bold uppercase tracking-wider px-2 py-0.5 rounded bg-surface-sunken text-primary border border-primary/30">
                      {action.badge}
                    </span>
                    <span className="text-xs font-semibold text-ink-subtle group-hover:text-primary group-hover:translate-x-0.5 transition-all">
                      Run →
                    </span>
                  </div>

                  <h4 className="text-sm font-bold text-ink group-hover:text-primary transition-colors leading-snug">
                    {action.title}
                  </h4>

                  <p className="text-xs text-ink-soft line-clamp-2 mt-1.5 leading-relaxed">
                    {action.detail}
                  </p>
                </div>
                ...
                <div className="mt-4 pt-3 border-t border-line flex items-center justify-between">
                  {action.visualType === "sales-trend" && (
                    <div className="w-full flex items-center justify-between text-[11px] text-ink-subtle">
                  ...
                  {action.visualType === "stock-health" && (
                    <div className="w-full flex items-center justify-between text-[11px] text-ink-subtle">
                      <span className="text-primary">Inventory check</span>
                      <div className="flex gap-1">
                        <div className="w-4 h-2 rounded bg-success" title="Healthy Stock" />
                        <div className="w-3 h-2 rounded bg-warning" title="Low Threshold" />
                        <div className="w-2 h-2 rounded bg-danger" title="Depleted" />
                      </div>
                    </div>
                  )}
                  ...
                  {action.visualType === "listing-draft" && (
                    <div className="w-full flex items-center justify-between text-[11px] text-ink-subtle">
                      <span className="text-ink-soft">Silk Saree Copy</span>
      ...
      <Card
        ...
        padding="none"
        radius="xl"
        className="border-line bg-surface shadow-md overflow-hidden flex flex-col h-[620px]"
      >
        {/* Workspace Top Bar */}
        <div className="flex items-center justify-between px-5 py-3.5 border-b border-line bg-surface-sunken/60">
          <div className="flex items-center gap-2.5">
            <div className="size-7 rounded-lg bg-surface border border-line grid place-items-center">
              <Bot className="size-4 text-primary" />
            </div>
            <div>
              <div className="flex items-center gap-2">
                <span className="text-xs font-bold text-ink">Copilot Active Session</span>
        ...
        <div className="flex-1 overflow-y-auto p-4 sm:p-6 space-y-4 bg-canvas/40">
            ...
            <div key={msg.id} className="space-y-2">
              {msg.role === "user" ? (
                ...
                <div className="flex justify-end">
                  <div className="max-w-[85%] sm:max-w-[75%] rounded-2xl rounded-tr-xs bg-primary px-4 py-2.5 text-sm font-medium text-[#181714] shadow-xs">
                    {msg.content}
                  </div>
                </div>
              ) : (
                ...
                <div className="flex flex-col gap-2 max-w-[92%] sm:max-w-[85%]">
                  ...
                  {msg.toolCall && (
                    <div className="inline-flex items-center gap-1.5 self-start px-2.5 py-1 rounded-pill bg-surface-sunken border border-line text-xs text-ink-soft shadow-xs">
                  ...
                  <div className="rounded-2xl rounded-tl-xs border border-line bg-surface p-4 sm:p-5 text-sm text-ink shadow-xs space-y-3.5">
                    <div className="prose prose-sm dark:prose-invert max-w-none text-ink leading-relaxed whitespace-pre-line">
                      {msg.content}
                    </div>

                    {/* Structured Data: Merchant Sales Summary */}
                    {msg.structuredData?.type === "merchant_sales_summary" && (
                      <div className="p-4 rounded-xl border border-line bg-surface-sunken space-y-3">
                        <div className="flex items-center justify-between">
                        ...
                        <div className="grid grid-cols-2 sm:grid-cols-4 gap-2.5 pt-1 text-center">
                          <div className="p-2.5 rounded-lg bg-surface border border-line">
                            <span className="text-[11px] text-ink-subtle block">Gross Revenue</span>
                            <span className="font-display text-base font-bold text-ink">
                              {formatPrice(msg.structuredData.data.todaySales)}
                            </span>
                          </div>
                          <div className="p-2.5 rounded-lg bg-surface border border-line">
                            <span className="text-[11px] text-ink-subtle block">Total Orders</span>
                            <span className="font-display text-base font-bold text-ink">
                              {msg.structuredData.data.todayOrders}
                            </span>
                          </div>
                          <div className="p-2.5 rounded-lg bg-surface border border-line">
                            <span className="text-[11px] text-ink-subtle block">Pending Dispatch</span>
                            <span className="font-display text-base font-bold text-primary">
                              {msg.structuredData.data.pendingOrdersCount}
                            </span>
                          </div>
                          <div className="p-2.5 rounded-lg bg-surface border border-line">
                            <span className="text-[11px] text-ink-subtle block">Low Stock Alert</span>
          ...
          {streamingText && (
            <div className="flex flex-col gap-2 max-w-[85%]">
              <div className="rounded-2xl rounded-tl-xs border border-line bg-surface p-4 text-sm text-ink shadow-xs">
          ...
          {isThinking && !streamingText && (
            <div className="flex items-center gap-2.5 text-xs text-ink-soft bg-surface-sunken p-3 rounded-xl border border-line w-fit shadow-xs">
              <Loader2 className="size-3.5 animate-spin text-primary" />
              {activeToolName ? (
                <span>
                  Querying store service: <strong className="font-mono text-primary">{activeToolName}</strong>…
                </span>
              ) : (
        ...
        <div className="p-3.5 sm:p-4 border-t border-line bg-surface-sunken/70">
          <form onSubmit={handleSubmit} className="flex items-center gap-2">
            <input
              ...
              disabled={isThinking}
              className="flex-1 rounded-xl border border-line bg-surface px-4 py-2.5 text-sm text-ink placeholder:text-ink-subtle focus:border-primary focus:outline-none transition-colors"
            />
